...
class Event(Base):
    """Track custom events (product views, add to cart, etc.)"""
    __tablename__ = "events"

    id = Column(Integer, primary_key=True, index=True)
    visit_id = Column(Integer, ForeignKey("visits.id"))
    event_type = Column(String, nullable=False)  # 'product_view', 'add_to_cart', etc.
...
class SEOConnection(Base):
    __tablename__ = "seo_connections"

    id = Column(Integer, primary_key=True, index=True)
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=False)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    google_email = Column(String, nullable=False)
    site_url = Column(String, nullable=True)  # sc-domain:... or https://...
    ...
    tokens = relationship("SEOToken", back_populates="connection", uselist=False)


class SEOToken(Base):
    __tablename__ = "seo_tokens"

    id = Column(Integer, primary_key=True, index=True)
    connection_id = Column(Integer, ForeignKey("seo_connections.id"), nullable=False, unique=True)
