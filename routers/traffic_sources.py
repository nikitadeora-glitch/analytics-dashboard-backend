...
def get_exit_links(
    ...
    # Join with Visit model to get filter fields (use LEFT JOIN to include orphaned exit links)

    query = query.outerjoin(models.Visit, 
    ...
    # IP Address filter (only if Visit data exists)
    ...
    # Entry page filter (page_entry_page parameter) (only if Visit data exists)
    ...
    # Country/City filter (only if Visit data exists)
    ...
    # Browser filter (only if Visit data exists)

    if browser:

        query = query.filter(

            models.Visit.browser.isnot(None),

            models.Visit.browser.like(f"%{browser}%")
    ...
    # Device filter (only if Visit data exists)
    ...
    # Traffic sources filter (using referrer field from Visit) (only if Visit data exists)
    ...
    # Entry page filter (using entry_page from Visit) (only if Visit data exists)
    ...
    # Platform OS filter (using os field from Visit) (only if Visit data exists)
    ...
    # System platform OS filter (using os field from Visit) (only if Visit data exists)
    ...
    # Engagement exit link filter (using exit_page from Visit) (only if Visit data exists)
    ...
    if engagement_sessions_per_visitor is not None:
        ...
        # LEFT JOIN with subquery to include orphaned exit links
        ...
        # Apply filter based on operator (only filter if we have session data)
    ...
    if page_views_per_session is not None:
        ...
        # Apply filter based on operator (only filter if we have page view data)
