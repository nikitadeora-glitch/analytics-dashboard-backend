...
def get_most_visited_pages(
    ...
    engagement_session_length_min: Optional[float] = None,

    engagement_session_length_max: Optional[float] = None,

    engagement_session_length_operator: Optional[str] = None,

    engagement_page_views_per_session_value: Optional[int] = None,

    engagement_page_views_per_session_operator: Optional[str] = None,

    engagement_sessions_per_visitor_value: Optional[float] = None,

    engagement_sessions_per_visitor_operator: Optional[str] = None,

    engagement_conversions_value: Optional[int] = None,

    engagement_conversions_operator: Optional[str] = None,

    engagement_exit_link: Optional[str] = None,

    traffic_traffic_sources: Optional[str] = None,
    ...
    page_last_page_of_session: Optional[str] = None,

    location_country_city: Optional[str] = None,

    location_language: Optional[str] = None,

    location_ip_address: Optional[str] = None,

    location_isp: Optional[str] = None,

    system_platform_os: Optional[str] = None,

    system_browser: Optional[str] = None,
    ...
    try:
        ...
        for base_url, total_views, unique_sessions, avg_time_spent, title in page_stats:
            ...
            # Get actual visits for this most visited page

            visits_for_page = []

            # Get all PageView records for this page URL with visit data
            page_views_query = db.query(models.PageView).options(
                joinedload(models.PageView.visit)
            ).join(models.Visit).filter(
                models.Visit.project_id == project_id,
            ...
            # Get all page views and convert to visit-like objects
            page_views_data = page_views_query.order_by(desc(models.PageView.viewed_at)).all()
            
            # Group page views by visit_id to create visit entries
            visit_groups = {}
            for pv in page_views_data:
                if pv.visit_id not in visit_groups:
                    visit_groups[pv.visit_id] = {
                        "session_id": pv.visit.session_id if pv.visit else str(pv.visit_id),
                        "visitor_id": pv.visit.visitor_id if pv.visit else None,
                        "visited_at": pv.visit.visited_at.isoformat() if pv.visit and pv.visit.visited_at else pv.viewed_at.isoformat(),
                        "time_spent": pv.visit.session_duration if pv.visit else pv.time_spent or 0,
                        "country": pv.visit.country if pv.visit else None,
                        "city": pv.visit.city if pv.visit else None,
                        "device": pv.visit.device if pv.visit else None,
                        "browser": pv.visit.browser if pv.visit else None,
                        "os": pv.visit.os if pv.visit else None,
                        "ip_address": pv.visit.ip_address if pv.visit else None
                    }

            # Convert visit groups to array

            visits_for_page = list(visit_groups.values())
